...
public class ExtentReporter {
	...
	private static File configFile = new File(System.getProperty("user.dir")
			+ File.separator + "ReportConfig.xml");
	 ...
	 * @param extentTest
	 */
	private static void setInterruptedTestStatus(ITestResult iTestResult,
			ExtentTest extentTest) {
		if (!(extentTest.getRunStatus().equals(LogStatus.UNKNOWN) || extentTest
				.getRunStatus().equals(LogStatus.PASS))) {
			return;
		}
			...
			if (iTestResult.getThrowable() == null)
				...
				extentTest.log(
						...
						"<div class=\"stacktrace\">"
								+ ExceptionUtils.getStackTrace(iTestResult
										.getThrowable()) + "</div>");
			break;
		case 3:
			if (iTestResult.getThrowable() == null)
				extentTest.log(LogStatus.SKIP,
						"<font color=\"orange\">Test Skipped</font>");
			else
				extentTest.log(
						LogStatus.SKIP,
						"<div class=\"stacktrace\">"
	...
	private static synchronized ExtentReports getReportInstance(
		...
		if (report == null) {
			String reportFilePath = new File(iTestResult.getTestContext()
	...
	private static ExtentTest startTest(String description) {
		...
		String testName = iTestResult != null ? getTestName(iTestResult)
				: Thread.currentThread().getName();
		Integer hashCode = iTestResult != null ? iTestResult.hashCode()
		...
		if (tests.containsKey(hashCode)) {
			...
			if (iTestResult == null || !iTestResult.getMethod().isTest()) {
				test = new ExtentTest(testName, description);//testName, description);
			} else {
				String className = iTestResult.getTestClass().getName();
				String[] categName = className.split("\\.");
				int categ = categName.length;
				test = getReportInstance(iTestResult).startTest(testName,description).assignCategory(categName[categ-1]);//.getMethod().getGroups());
				
				//iTestResult.getTestClass().getName()
				/*test = getReportInstance(iTestResult).startTest(testName,
						description).assignCategory(
	...
	public static void testCaseInfo(String testCaseInfo) {
		startTest("<strong><font size = \"4\" color = \"#000080\">"
	...
	public static void debug(String event) {
		getTest().log(LogStatus.INFO, event);
	}
	...
	public static void pass(String passMessage) {
		getTest().log(LogStatus.PASS,
				"<font color=\"green\">" + passMessage + "</font>");
	}
	...
	public static void logStackTrace(Throwable t) {
		if (t instanceof SkipException) {
			getTest().log(
					LogStatus.SKIP,
					"<div class=\"stacktrace\">"
							+ ExceptionUtils.getStackTrace(t) + "</div>");
		} else {
			getTest().log(
					LogStatus.FAIL,
					"<div class=\"stacktrace\">"
					...
					.forEach(
							log -> {
								if (log.getLogStatus() == LogStatus.ERROR
										|| log.getLogStatus() == LogStatus.FAIL
	 ...
	 * @param outdir
	 */
	public static void closeReport(List<ITestResult> allTestCaseResults,
			String outdir) {
		if (isReportClosed) {
			return;
		}
		if (report == null && allTestCaseResults.size() > 0) {
			...
			report = new ExtentReports(outdir + File.separator
					+ "ExtentReport.html", true);
			if (configFile.exists()) {
				report.loadConfig(configFile);
			}
			report.endTest(report
					.startTest("Empty TestNG Suite",
		...
		if (report != null) {
			...
			for (ITestResult iTestResult : allTestCaseResults) {
				...
				if (!tests.containsKey(hashCode)) {
					extentTest = report.startTest(testName, iTestResult
							.getMethod().getDescription() == null ? ""
							: iTestResult.getMethod().getDescription());
					extentTest.setStartedTime(getTime(iTestResult
							.getStartMillis()));
					extentTest.assignCategory(iTestResult.getMethod()
							.getGroups());
					...
					if (extentTest.getEndedTime() == null) {
						setInterruptedTestStatus(iTestResult, extentTest);
						extentTest.setEndedTime(getTime(iTestResult
								.getEndMillis()));
					}
				}
			}
